...
```{r}
library(tidyverse)
pheno = readr::read_csv('https://data.ecoforecast.org/targets/phenology/phenology-targets.csv.gz')
pheno %>%
  filter(siteID == 'HARV') %>%
  select(time, gcc_90, gcc_sd) %>%
  mutate(gcc_low = gcc_90 - gcc_sd,
         gcc_high = gcc_90 + gcc_sd) %>%
  ggplot(aes(x = time, y = gcc_90, ymin = gcc_low, ymax = gcc_high)) +
  geom_ribbon(color = 'light pink') +
...
```{r}
ggplot() +
  geom_line(aes(x = time, y = EVI))

time_evi = time

pheno %>%
  filter(time %in% time_evi) %>%
  filter(siteID == 'HARV') %>%
  ggplot() +
  geom_line(aes(x = time_evi, y = EVI, color = 'EVI')) +
  geom_line(aes(x = time, y = gcc_90, color = 'GCC')) +
  xlab('Time') + ylab('Greenness') +
  scale_color_discrete(name = 'Greeness Index')
```
...
```{r}
s3 <- arrow::s3_bucket(bucket = "neon4cast-drivers/noaa/gefs-v12/stage2/parquet",
                        endpoint_override = "data.ecoforecast.org",
                        anonymous = TRUE)
ds <- arrow::open_dataset(s3, partition = c('start_date', 'cycle'))
df <- ds %>% 
  filter(start_date == '2022-09-06',
         cycle == 0, 
         variable=="air_temperature",
         site_id == "HARV") %>%
  collect()
df %>%
  select(predicted, ensemble, time) %>%
  ggplot(aes(x = time, y = predicted, color = as.factor(ensemble))) +
  geom_line() +
...
Species 1 is Gloeotrichia echinulata and species 2 is Anabaena circinalis. The units are number/m\textsuperscript{2}. Yes, the forecast propagated the intial condition uncertainty (initial_conditions: status: present).
...
f <- system.file(package = "cronR", "extdata", "getmodisdata.R")
cmd <- cron_rscript(f)
cron_add(command = cmd, frequency = 'weekly', at='7AM', id = 'test2')
